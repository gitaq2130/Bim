...
async def create_daily_report(project_id: str, request: Request, session: Session = Depends(get_session),
                              ...
                              _: ProjectContext = Depends(require_project_role("contractor"))) -> DailyReportResponse:
    """작업일보 입력(JSON 또는 multipart report+photos, 그 프로젝트의 contractor 만).
    상태기계 apply_daily_report → 3중 검증 불일치 시 검토요청·전이 보류."""
...
def list_daily_reports(project_id: str, session: Session = Depends(get_session), _: ProjectContext = Depends(require_project_role())) -> list[DailyReportView]:
