...
- **줄바꿈**(§6-1 ③). 연산자에서 줄이 갈리면(`187 >=` / 다음 줄 `187`) 이 정규식이 못 본다.
- **바닥값을 왼쪽에 적은 비교식.** 오른쪽 규칙이 그것을 잘못 읽는다. 그래서 이 세 파일의 규약은
  **바닥값을 오른쪽에 적는다**이고, 어기면 이 테스트가 빨개진다 — **잘못 잡는 쪽(거짓 양성)이라
  안전한 방향**이다.
- **소유 밖.** `docs/plans/`·`docs/adr/` 는 §3-13 첫째 갈래의 **기록물**이라 대상이 아니다. 실측:
  `docs/plans/0011-*.md` 가 옛 바닥값의 비교식을 갖고 있고, 그것은 그 계획 §0 이 못박은 트리의 값이다
  (소급 갱신하지 않는다).
- **이 파일 자신.** 아래 `SCANNED_ROOTS` 훑기에서 **자기를 뺀다** — 이 docstring 이 예시로 적는 비교식이
  전수 단언을 자기가 깨기 때문이다. 뺄 수 있는 근거는 이 파일이 바닥값을 **복창하지 않는다**는 것이다:
  값은 `axis.read_floor()` 로만 온다(부재 단정 — 이 파일에 바닥값 리터럴이 없다).

## 결함 있는 상태에서 실제로 죽는가 (§6-2 1 — 변이 실측)

변이는 **한 자리씩** 심고 `git diff` 로 적용을 확인한 뒤 재고 원복했다(무력 변이 방지). 명령은 네 번
모두 `pytest tests/invariants/test_postgres_floor_recitation.py -q`.
