...
이 절의 게이트는 ADR 0016 의 `## 3.`(그 ADR 이 계약으로 붙드는 것들) 중 **`EXPLAIN` 을 요구하는
조항**이다. 찾는 명령은 그 자리에서 돈다: `grep -n "EXPLAIN" docs/adr/0016-*.md` — 그 히트 중
`## 3.` 아래의 것이 그 조항이다. **그 조항의 문안은 여기 인용하지 않는다** — 아래 문단은 *이 파일이
한 측정*의 서술이지 그 조항의 인용이 아니다. 서수(「§3 2」)로도 부르지 않는다: 그 절에 조항이
하나 앞에 끼면 서수가 조용히 낡는다(CLAUDE.md §3-13 둘째 갈래 — 계속 편집되는 자리는 인용하지 말고 그 자리에서 도는
참조로 가리킨다. §3-13 이 `.claude/agents/reviewer.md` 의 불릿을 서수로 부르지 않는 것과 같은 이유다).
**근거는 이 자리의 실측이다**: 여기 있던 인용은 그 조항의 **초판 문안**이었는데 리뷰 라운드 3
(`3c6ac58`)이 조항을 좁혔고, 인용은 현재형이라 **그 커밋으로 거짓이 됐다**(CLAUDE.md §6-3 9회차 —
「내 작업이 남의 파일을 낡게 만드는 자리」. `tests/` 는 그 라운드가 만질 수 없는 소유였고 §후속으로도
넘어가지 않았다). 같은 ADR 은 이 사이클에도 계속 고쳐지고 있으므로, **새 문안을 다시 베끼는 것은
같은 결함을 다음 커밋에 다시 심는 일이다** — 그래서 문안이 아니라 참조를 둔다.

**이 파일이 한 측정은 이것이다: 안 죽는 칸의 이름을 저장소 밖 탐침이 아니라 그 칸이 관측된 통합
세션 안에서, 그 세션이 고른 계획마다 하나씩 잡았다.** 이 문장은 **이 파일에 대한 사실**이라 ADR 이
다시 고쳐져도 낡지 않는다 — 그 조항이 이 측정을 여전히 만족으로 받는지는 위 grep 으로 읽어 판정한다.
**초판이 붙인 이름은 저장소 밖 2~3행 탐침의 것이었고, 그 탐침은 통합 세션의 실측을 틀리게
예측한다:**
...
- **초판의 전칭 부정은 거짓이다** — *"어떤 픽스처도 이 두 칸을 죽일 수 없다 … 갈리게 하려면 정렬 키를
  ...
  ② ADR 0016 §4 는 *"§후속 16 이 `tests/unit` 을 postgres 로 옮겨 행 수를 키울 때 **그 값이
  올라가야 한다**"* 라고 적고, §6 대안 8 (*"행을 늘려 플래너가 계획을 바꾸게 만든다"*)을
  **불가능이 아니라 다른 근거로** 기각한다
...
- **그래서 이 두 자리에서 계약을 실제로 붙드는 것은 역방향 변이 쪽이다** — A3·A5 모두 두 축에서
  **4/4** 죽는다. 역방향은 엔진이 어떤 계획을 고르든 **다른 답을 강제**하기 때문이다
  (ADR 0016 §2-3 정정 상자: *"역방향 변이가 안 죽으면 그 회귀는 장식이다"*).
