...
def _store_scan(session, gid: str, state: ScanState) -> None:
    session.add(ScanVerdictRow(scan_id="scan-1", global_id=gid, state=state.value, confidence=0.9,
...
def test_completed_report_vs_not_built_scan_creates_ver_001_and_keeps_state(session, seeded):
    ...
    logic = build_logic_context(session, gid)
    ...
    assert session.get(BimObjectRow, gid).state == ObjectState.PLANNED.value
...
def test_apply_daily_report_blocks_completion_on_mismatch(session, seeded):
    gid = seeded["expected"]["A100"][0]
    _store_scan(session, gid, ScanState.NOT_BUILT)
    ...
    assert session.get(BimObjectRow, gid).state == ObjectState.REPORTED.value
    ...
    assert session.get(BimObjectRow, gid).state == ObjectState.REPORTED.value
    assert session.query(StateTransitionRow).filter_by(global_id=gid).count() == 1
...
def test_consistent_completion_reaches_inspection_requested(session, seeded):
    ...
    _store_scan(session, gid, ScanState.ESTIMATED_DONE)
    sm = ObjectStateMachine()
