...
| 누가 | 언제 | 무엇을 |
|---|---|---|
| `architect` | 계획·ADR 을 쓸 때 | §6-1 생성 기준·블라인드 스팟 명시 + 한 건 실측, §6-3 한정어 역방향 확인을 **문서에 적고 실행 표로 남기기**(새로 쓴 조건절 포함, “옛 조건이 잡던 것”까지, **각 칸을 참조가 아니라 실행값·코드 인용으로 채우고 같은 문서의 인접 절과 교차 확인**), §6-4 문구 정정을 Deferred 로 넘기지 않기 |
