...
- **같은 프로젝트 안에서 두 공정표가 같은 `activity_id` 를 쓰는 경우.** 복합 PK 이후에도 나중 업로드가
  ...
  14차 리뷰가 이 항목을 실측으로 두 가지 보강했다. ① 인계 결과 A110 의 선행공정이 두 공정표의 관계를
  합친 `["A100","A400"]` 이 되는데, **부모 커밋에서도 동일**하다 — 이 변경으로 나빠지지 않았다(오히려
  교차 프로젝트만큼은 좁아졌다). 구현 중 "이번 변경으로 조금 나빠진다"는 보고가 있었으나 실행으로
  뒷받침되지 않았다. ② 더 중요한 것: **같은 파일을 다시 올려도 `schedule_id` 가 `file_id` 에서 파생돼
  매번 새로 생기므로 `save_schedule` 의 "교체" 분기가 실사용 경로에서 아예 타지 않는다.** 4회 재업로드
  실측 결과 `ScheduleRow` 1/2/3/4, 관계 5/10/15/20 으로 누적되고 Activity 만 6으로 유지(인계)된다.
  따라서 §Decision 2 의 "공정표를 다시 올릴 때마다 삭제·재생성된다"는 서술은 Activity 에는 맞지만
  **관계에는 맞지 않는다.** 이 역시 부모 커밋에서도 동일한 선존재 동작이다.
