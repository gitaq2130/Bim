...
| P8 | 대장측 **제목** 오타 정정 | 음성(rename 경로 — `doc_id` 가 바뀌어 고아가 된다) | 동일 | 오탐 아님 |
...
5. **(개정 1 / 개정 2에서 무게가 커졌다) `cause` 값의 정본이 **네 곳**에 복제돼 있다.** 지금
   ...
   **그때까지의 방어는 폴백 규칙이다**: 세 소비자 모두 모르는 `cause` 를 `unspecified` 로
   두고 **`orphaned` 로 떨어뜨리지 않는다**(모르는 것을 고아라고 적으면 §5-4 가 고치려는 바로 그 거짓이
   된다). 새 경위를 추가하거나 이름을 바꾸는 변경은 **네 자리를 함께** 고쳐야 하며, 그 사실을 여기 적어
   둔 것이 지금의 유일한 강제 장치다(계획 0003 §12-f 체크 3 이 리뷰어에게 같은 목록을 요구한다).
