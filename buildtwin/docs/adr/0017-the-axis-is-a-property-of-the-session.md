...
**[정정 2 — 리뷰 사이클 0013 라운드 2, 마감 뒤. 위 표와 1·2·3 은 고치지 않는다(CLAUDE.md §3-13
첫째 갈래 — 그 값들은 `87d743b` 트리·N=1 의 기록이고 그 자리에서는 참이다).]**

**이 정정의 실측이 나온 자리.** HEAD **`972ffd2`**, 저장소 루트 `/home/user/Bim`, 루트
`git status --porcelain` 빈 출력. **DB**: 측정마다 **새 `initdb`**(root 는
`initdb: error: cannot be run as root` — `su postgres`), `PostgreSQL 16.13 (Ubuntu
16.13-0ubuntu0.24.04.1)`, PostGIS 없음, `autovacuum=on · naptime=60 · threshold=50 ·
scale_factor=0.2 · insert_threshold=1000`(전부 `pg_settings`). 포트
**55431·55432·55433·55434·55436·55437·55438·55439·55440**(착수 검사 — `shutil.which('ss')` → `None`,
`/proc/net/tcp6` **부재**, `/proc/net/tcp` 의 `st=0A` 에 55430~55459 **0건**, `socket.bind`
**양성 검사** 여덟 개 전부 성공. 그런데 55434 에서 `pg_ctl` 이 `could not bind IPv4 address
"127.0.0.1": Address already in use` 로 죽어 **그 포트를 버리고 55436 으로 옮겼다** — 양성 검사도
경합을 없애지 못한다, 계획 0013 §후속 46 에 값으로 붙는다). 매 측정은 `pytest tests/unit -q`
**전량 492 passed** · `[db-axis] … tests_on_postgres=252 engines=252 floor=252` · 끝에 남은
`bt_test_%` 스키마 **0** · 클러스터 정지·삭제·포트 반납.

**위 표의 「실행 중 **2**」는 계약이 될 수 없다 — 그것은 3 이 카탈로그 값에 대해 적은 것과 같은
종류의 스냅샷이고, 게다가 표본 간격에 걸린다.** 같은 워크로드를 간격을 바꿔 **여덟 번** 다시 쟀다:

| 표본 간격 | N | 표본 수 | 사용자 테이블 `max(n_dead_tup)` | 그 표본의 관계 · 같은 표본의 `n_live_tup` |
|---|---|---|---|---|
| **0.2초**(위 표가 쓴 간격) | 4 | 365 · 400 · 376 · 430 | **8 · 6 · 2 · 127** | `review_requests`(0) · `bim_objects`(42) · `activities`(6) · `bim_objects`(**0**) |
| **0.02초** | 3 | 3,280 · 3,734 · 3,323 | **127 · 127 · 127** | 셋 다 `bim_objects`(**0**) |
| **0.01초** | 1 | 7,646 | **127** | `bim_objects`(**0**) |
| **0.005초** | 1 | 12,751 | **127** | `bim_objects`(**0**) |

**셋이 이 표에서 나온다.**

ⓐ **2 는 그 분포의 최저값이고, 위 표의 N=1 이 하필 최저값을 뽑았다.** 같은 **0.2초** 간격에서
네 번 재니 {2, 6, 8, **127**} 이다 — **간격을 바꾸지 않아도 127 이 나온다.** 간격을 10·20·40배로
좁히면 **다섯 번 중 다섯 번** 127 이고, 12,751 표본에서도 127 을 넘지 않는다. 그러므로 **127 은 이
워크로드의 실제 봉우리**이고 0.2초 간격은 그 봉우리에 걸리는지 마는지의 동전 던지기다.

ⓑ **그 봉우리는 문턱을 넘는다.** 봉우리 표본의 `n_live_tup` 이 **0** 이라 그 관계의 문턱은
`50 + 0.2 × 0 = 50` 이고 **127 > 50** 이다. 그러므로 §8-3 첫째 불릿의 근거 문장(*"뒤집으려면
사용자 테이블의 압력이 문턱을 넘어야 하는데, 실행 중 실측이 2 다"*)은 **거짓이다.** 처분은 §8-3 의
정정 2 에 있다 — 여기서 가리키기만 하지 않고 **그 자리에 값을 다시 적는다**(§6-3: 다른 절 참조로
갈음하지 않는다).

ⓒ **봉우리에는 이름이 있다.** 표본마다 `nodeid` 를 함께 읽으니(저장소 밖 pytest 플러그인 ·
0.01초 · 7,646 표본 · `tests/` 를 한 글자도 만지지 않았다, N=1) 127 이 서는 자리는
`tests/unit/ingest/test_persistence.py::test_same_ifc_uploaded_to_two_projects_is_the_whole_point_of_adr_0005`
하나다.

**그리고 3 의 「결정적」 목록도 좁아진다.** 3 은 `n_tup_del` · `n_tup_upd` · 「autovacuum 이 돈 관계
열 개」 셋을 *"두 측정에서 자릿수까지 같다"* 로 묶었는데, **표본이 둘에서 여덟이 되니 그 셋이
둘로 갈린다**:

| 값 | 8회 중 | 판정 |
|---|---|---|
| 카탈로그 `n_tup_del` | **405,216 — 8/8** | 워크로드가 만든 수. **결정적이다** |
| 사용자 `autovacuum_count` · `vacuum_count` | **0 / 0 — 8/8** | **결정적이다** — §8-3 정정 2 가 이 값 위에 선다 |
| 동시 생존 사용자 관계 최대 | **24 — 8/8** | **결정적이다** |
| 빈 클러스터 기준선 카탈로그 `max(n_dead_tup)` | **4 — 9/9**(새 `initdb` 아홉) | **결정적이다** |
| 카탈로그 `sum(autovacuum_count)` | **10 — 4/8** · **0 — 4/8** | **스냅샷이다.** 전량이 71~86초인데 `naptime=60` 이라 그 주기가 실행 안에 드는지가 갈린다 |
| 카탈로그 `n_tup_upd` | **13,761 — 4/8** · **13,612 — 4/8** | **위 칸에 따라온다**(autovacuum 이 `pg_class` 를 갱신한다). 결정적이지 않다 |
| 사용자 `max(n_live_tup)` | **42 — 7/8** · **10 — 1/8** | 같은 표본 간격 문제다 |
| 카탈로그 `max(n_dead_tup)` | **7,170 ~ 186,732**(8회 범위) | 3 이 적은 그대로 — **어느 수도 계약이 아니다** |

**3 이 못박은 명제는 유지되고, 그것이 기대던 「자릿수까지 같다」가 셋에서 하나로 준다** —
`n_tup_upd`=13,761 과 「돈 관계 열 개」는 **표본 둘에서 같았을 뿐**이고 여덟에서는 절반이다.
3 이 묶은 셋은 `n_tup_del`·`n_tup_upd`·「돈 관계 열 개」이고 **그중 둘이 떨어지므로 남는 것은
`n_tup_del` 하나**다. **표본 간격은 값과 함께 적는다**(계획 0013 §후속 34 의 실례 — 새 번호를
열지 않는다).

**[정정 3 — 리뷰 라운드 3. 위 문단의 「셋에서 둘로」를 「셋에서 하나로」로 고쳤다 — 소급 갱신이
아니다.]** 그 수는 **트리에 매인 실측이 아니라 바로 위 표에 대한 산술**이라 §3-13 첫째 갈래가
보호하는 종류가 아니고, 같은 문단이 *"둘이 떨어졌다"* 고 적으므로 **자기 표와 어긋난다**
(CLAUDE.md §6-3 의 *"같은 문서에서 다르게 말하는 줄이 있는지 먼저 찾는다"*, §6-4 1 의
*"사실과 다른 문구는 그것을 만든 사이클이 고친다"* — 그 문구를 만든 것은 이 사이클의 `fa9f42d`
이고 소유는 `architect`, 내 것이다). 리뷰어의 8회 재측정도 같은 판정이다.
**그리고 이 오독은 저장소 밖으로 한 번 전파됐고 그 자리는 고칠 수 없다** — 이 라운드의 재심사
지시문이 그것을 *"삼종 세트가 둘로 줄어든다"* 로 옮겨 적었다(오케스트레이터가 자기 잘못으로
적었다). 지시문은 저장소에 없는 산출물이라 **정정이 닿지 않는다**; 그 사실을 여기 값으로 적는
것이 이 사이클이 할 수 있는 전부다 — **§8-5 2 정정 3 의 「고칠 수 없는 자리」와 같은 모양**이고,
둘 다 계획 0013 §후속 **62** 의 표본이다(그 항목은 *"뒤집은 쪽이 문서에 적는다"* 로 닫힐
후보인데, **뒤집은 쪽이 저장소 밖일 때는 그 갈래도 닫지 못한다** — 62 에 값으로 붙인다).
...
**[정정 2 — 리뷰 사이클 0013 라운드 2, 마감 뒤. 위 세 불릿은 고치지 않는다(§3-13 첫째 갈래).
실측이 나온 자리·방법·N 은 §8-2 의 정정 2 에 있다.]**

**결정은 그대로다 — `autovacuum` 을 끄지 않는다. 뒤집히지 않는 *이유*가 바뀐다: 문턱 비교가 아니라
기전이다.**

- **첫째 불릿의 근거 문장이 거짓이다.** *"뒤집으려면 사용자 테이블의 압력이 문턱을 넘어야 하는데,
  실행 중 실측이 **2** 다"* → **넘는다.** 사용자 테이블 `max(n_dead_tup)` 의 봉우리는 **127**
  (`bim_objects`, 같은 표본의 `n_live_tup`=**0** → 문턱 `50 + 0.2 × 0 = 50`)이고, 0.02·0.01·0.005초
  간격 다섯 번 중 **다섯 번**, 0.2초 네 번 중 **한 번** 잡힌다. **결정을 지탱하는 것은 이제 이것이다**:
  사용자 테이블의 `autovacuum_count` · `vacuum_count` 가 **여덟 번 전부 0 / 0** 이고, 까닭은
  `autovacuum_naptime=60` 인데 이 트리의 격리가 **테스트마다 새 스키마**라 스키마 수명이 초 단위
  이하이고 **naptime 주기가 그 테이블에 닿기 전에 `DROP SCHEMA … CASCADE` 가 되기** 때문이다.
  **이 트리에서 「문턱을 넘는다」와 「autovacuum 이 돈다」는 다른 사건이고, 결정이 걸린 것은 뒤쪽이다.**
- **그래서 둘째 불릿이 못박은 재개 조건의 *첫째 갈래를 지운다*.** 그 문안
  (*"`pg_catalog` 를 뺀 관계에서 `n_dead_tup` 이 문턱을 넘거나, 사용자 테이블에서 autovacuum 이
  실제로 도는 것이 관측될 때"*)은 **첫째 갈래가 오늘 이미 참**이다 — 이 마감은 재개 조건을 좁혀
  놓고 **그 좁힌 조건이 자기 워크로드에서 이미 발화한다는 것을 보지 못했다.** 그대로 두면 다음
  사이클이 127 을 재고 §후속 30 을 다시 연다. CLAUDE.md §6-2 1 의 물음(*"이 단언의 기대값을,
  결함 있는 코드가 그대로 만족하는가"*)을 **재개 조건에 걸면** 첫째 갈래는 두 상태를 구별하지 못한다.
  **정정된 재개 조건**: *"`pg_catalog` 를 뺀 관계에서 `autovacuum_count` 또는 `vacuum_count` 가
  **1 이상**인 것이 관측될 때."* **다음 사이클은 이 문안으로 읽는다**(§2-4 역방향 확인의
  *"재측정이 ㉠ 을 뒤집으면"* 도 이 문안으로 좁힌다).
- *역방향 확인 — 문턱 갈래를 지우면 무엇을 잃는가(§6-3: 옛 조건이 잡던 것까지 묻는다).* 잃는 것은
  **압력 자체의 관측**이다. 그러나 그 관측은 ① **표본 간격에 걸려** 같은 워크로드에서 2 와 127 을
  오가고(§8-2 정정 2 ⓐ) ② **오늘 이미 참**이라 두 상태를 가르지 못한다. 옛 조건이 잡던 것은
  「압력이 처음 문턱을 넘는 날」인데 **그날은 이미 지났고**, 넘은 채로 아무 일도 일어나지 않는 것이
  오늘의 관측이다. 남는 갈래는 압력이 실제로 **소비되는** 순간만 잡는다 — 그 순간이 곧
  `autovacuum` 을 끄는 것이 사용자 테이블의 유지를 함께 끄는 순간이다.
- *새 조건을 참으로 만들 수 있는 것이 무엇인지도 값으로 적는다.* 스키마 수명이 `naptime`(60초)을
  넘는 격리 — 즉 **세션 스키마**(계획 0013 §후속 59)다. 이 트리의 전량은 **71~86초**(여덟 번)이므로
  세션 스키마에서는 주기가 사용자 테이블에 닿을 수 있다. **§후속 59 는 이제 §후속 31 과 §후속 30 의
  재개를 함께 진다.**
- **위 「한정 셋」의 ②를 갱신한다**(①③ 은 그대로다): *"N=1 이다(전량 실행 하나 · 표본 472)"* 는
  이 정정으로 **N=8 · 표본 365~12,751 · 표본 간격 넷(0.2 · 0.02 · 0.01 · 0.005초)** 이 됐다.
...
2. **`.github/workflows/buildtwin-ci.yml:82` 의 주석 *"확장은 만들지 않는다(§후속 24)"* 는
   ...
   **[정정 2 — 마감 뒤. 위 문단은 고치지 않는다(§3-13 첫째 갈래).]** 이 항목이 §후속 **61** 로
   넘긴 빚은 **같은 사이클의 `972ffd2`(qa)가 갚았다** — 그래서 61 은 **닫혔다**(계획 0013 §M-9).
   그 커밋이 실측으로 더한 것 둘을 여기 싣는다: ⓐ 로드되는 DB 는 `POSTGRES_DB` **하나가 아니라
   `template_postgis` 와 `buildtwin_test` 둘**이고 각각 `CREATE EXTENSION` 넷이다, ⓑ 그것을 하는
   것은 `/usr/local/bin/docker-entrypoint.sh` 가 `sourcing /docker-entrypoint-initdb.d/10_postgis.sh`
   하는 것이며 **최초 init 때만** 돈다. **확장 이름은 잡 로그에 없다** — 그래서 `qa` 가 적지 않았고
   나도 짓지 않는다(CLAUDE.md §6-4 2: 모르는 값은 모른다고 적는다). 위 본문이 *"확장 넷"* 이라
   적은 것은 **로그가 세는 `CREATE EXTENSION` 의 수**이지 이름이 아니다.

   **그리고 위 본문의 `:82` 는 이 트리에서 다른 줄을 가리킨다 — 소급 갱신하지 않고 참조를 바꾼다.**
   `972ffd2` 가 그 주석을 고치며 아래를 **7줄** 밀었고, 거짓이던 절(*"확장은 만들지 않는다(§후속 24)"*)
   자체가 사라졌다. **`87d743b` 에서 `:82` 가 그 줄이라는 이 절의 못박음은 그 트리에서 참이므로
   고치지 않는다**(§3-13 첫째 갈래 · §0 이 HEAD 를 못박고 있다). 다만 **가리켜진 파일이
   `docs/` 가 아니라 계속 편집되는 자리**라 오늘의 독자에게는 줄 번호가 답을 주지 못한다 —
   §3-13 의 두 갈래는 **인용하는 자리**로 갈리는데 이 인용은 **인용되는 자리**가 둘째 갈래다.
   그래서 오늘 그 주석을 찾는 참조를 함께 적는다(계획 0013 §후속 55 ⓐ 의 형태):
   `grep -n "확장을 만들지는 않지만" .github/workflows/buildtwin-ci.yml` → **히트 하나**
   (**저장소 루트에서** 돈다 — 그 파일은 `buildtwin/` 밖이다).
   **파일을 적어야 한다** — 안 적으면 이 줄과 계획 0013 이 함께 히트한다(CLAUDE.md §2 의
   `grep -n "명시적 허가"` 가 같은 이유로 파일을 적는다).

   **[정정 3 — 리뷰 라운드 3. 「6줄」을 「7줄」로 고쳤다.]** 그 수는 **내가 재지 않고 `972ffd2` 의
   커밋 본문에서 옮겨 적은 것**이고(그 본문 *"내 편집이 그 아래를 6줄 밀었다"* — `git log -1
   --format=%B 972ffd2 | grep -n "6줄 밀었다"` → **85행**), 그것이 CLAUDE.md §6-1 이
   **「가리킴으로 확인을 갈음」** 이라 부르는 모양이다. **내가 직접 쟀다**(저장소 루트):
       ...
       git show --numstat 972ffd2 -- .github/workflows/buildtwin-ci.yml                     → 8  1

   두 앵커가 각각 **+7**(83→90 · 146→153)이고 편집은 **-1 +8 = +7** 이다. 병합 없음, 그 커밋이
   만진 파일은 이 하나다. **`972ffd2` 의 커밋 본문은 고칠 수 없다**(작성자 `qa`, 이미 푸시된
   히스토리) — 그러므로 이 사이클에는 **`6` 을 말하는 자리가 하나 남고 그 자리는 저장소에 남는다.**
   **전수의 기준**(CLAUDE.md §6-1 ①): 루트 `grep -rn "밀었\|밀어\|여섯 줄\|여섯줄" .`
   (`node_modules`·`design_handoff*` 제외) — 활용형·한글 수사까지 넓혔고, **히트 수가 아니라 각
   히트를 읽어** 걸렀다(대부분은 다른 뜻의 「표 밖으로 밀어냈다」다). *놓치는 것*: **커밋 본문은
   grep 이 닿지 않는다** — 그 자리는 `git log --format=%B` 로 따로 봤다. 결과, 「6」을 말하는
   자리는 **셋**이었고, 처분은 셋이 다르다: ⓐ **단정하던 두 자리**(이 절 본문 · 계획 0013 §M-9-4
   본문) → **고쳤다**. ⓑ **정정이 그 값을 인용하는 두 자리**(이 정정 3 · 계획의 정정 3) → 지우면
   정정이 무엇을 고쳤는지 사라지므로 **남긴다**. ⓒ **`972ffd2` 의 커밋 본문** → **고칠 수 없다**.
   ⓒ 가 있다는 것 자체가 계획 0013 §후속 **62** 의 표본이다(소유가 다른 두 커밋을 가로지른다 —
   `qa` 가 쓰고 `architect` 가 옮겼다).
