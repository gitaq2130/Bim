...
## 8. 정정·마감 (작업 8, architect — §0 의 예외 ㉯ 로 못박는다)

**이 절의 실측이 나온 자리.** 작업 트리 `/home/user/Bim/buildtwin`, 저장소 루트 `/home/user/Bim`,
브랜치 `claude/buildtwin-initial-setup-ubulzb`, **HEAD `87d743b`**(푸시됨), 루트
`git status --porcelain` 빈 출력. **이 절의 모든 값은 내가 이 트리에서 직접 다시 잰 것이다** —
`qa` 의 값을 옮기지 않았다(같은 값이 나온 칸도 "재현됐다"로 적지 "인용"으로 적지 않는다).
**DB**: 포트 **55443**(착수 실측 — `shutil.which('ss')` → `None`, `/proc/net/tcp` 의 `st=0A` 에서
55430~55459 **0건**, `socket.bind(("127.0.0.1", 55440..55447))` **여덟 개 전부 성공**,
`/proc/net/tcp6` **부재**). `su postgres` 로 띄운 `PostgreSQL 16.13 (Ubuntu 16.13-0ubuntu0.24.04.1)`,
`autovacuum=on · naptime=60 · threshold=50 · scale_factor=0.2 · insert_threshold=1000`(전부
`pg_settings`). root 로는 `initdb: error: cannot be run as root`. **측정 뒤 정지·삭제·포트 반납.**
**CI**: 이 절이 인용하는 CI 값은 내가 **원본 잡 로그를 내려받아** 읽은 것이다(`return_content=false`
로 받은 `logs_url` 을 `curl` — `unit` 잡 6,145줄 · `integration` 잡 1,027줄, 커밋 `87d743b`).

### 8-1. §3 완료 조건의 판정 (다섯 칸)

| 칸 | 판정 | 내가 다시 잰 값 |
|---|---|---|
| 1 `tests/unit` 이 축을 읽는다 | **충족** | 축 이름은 하나다(`grep -rnE "create_engine\(\|init_db\(\|database_url" tests/unit --include='*.py'` → **9줄**, 그중 `ingest/test_persistence.py:175` 는 docstring 이라 코드 **8** — 전부 `axis_db_url` 을 통과한다). 같은 명령을 `tests/invariants tests/regression` 에 → **0줄** |
| 2 누적값 단언이 테스트 함수에 없다 | **충족** | `git grep -n "tests_on_postgres" -- tests/` 의 히트를 각각 읽었다 — 테스트 함수 안 **0** |
| 3 바닥값 키가 트리별이고 안 돌린 트리의 키를 안 본다 | **충족 — 그리고 CI 러너에서도** | 로컬: `pytest tests/unit` **492 passed** `floor=252` · `pytest tests/integration` **226 passed** `floor=187`. **CI(`87d743b`)**: `unit` 잡 `[db-axis] tree=tests/unit … tests_on_postgres=252 engines=252 floor=252` · `integration` 잡 `[db-axis] tree=tests/integration … tests_on_postgres=187 engines=1 floor=187`(각 잡의 원본 로그에서 읽은 **전체 인용**) |
| 4 §후속 30 을 배선에서 다시 잰다 | **충족 — 값이 결정을 바꾸지 않는다**(§8-2) | 아래 표 |
| 5 §후속 31 을 N≥10 으로 잰다 | **충족(값) — 그러나 물음은 닫히지 않는다**(§8-3) | **10/10**(N=10, 매 회 `1 passed` + 계약인 `1 error`) |

### 8-2. **정정** — §2-4 ㉠ 의 근거 수는 배선의 것이 아니다 (§2-4 는 고치지 않는다)

**§2-4 본문을 소급 갱신하지 않는다**(CLAUDE.md §3-13 첫째 갈래 — 그 절은 `24d4aaa` 트리·탐침
격리의 기록이고 그 자리에서는 참이다). 정정은 여기 적는다.

| 값 | §2-4 ㉠·㉡(탐침, `24d4aaa`, 엔진마다 새 스키마 · **스키마를 안 지움**) | 착지한 배선에서 **내가** 다시 잰 값(`87d743b`, 포트 55443, N=1) |
|---|---|---|
| 사용자 테이블 `max(n_dead_tup)` — **전량 실행 뒤** | **42** | **0** — 그리고 이 0 은 "압력이 없다"가 아니라 **관측이 없다**: 테스트마다 `DROP SCHEMA … CASCADE` 라 끝난 시점에 사용자 관계가 **0개**이고, `pg_stat_all_tables` 의 그 행은 **테이블과 함께 사라진다** |
| 사용자 테이블 `max(n_dead_tup)` — **실행 중**(이 절이 새로 잰 것) | **잰 적 없다** | **2**(동시 생존 관계 최대 24 · `max(n_live_tup)`=**42** · 사용자 테이블 `autovacuum_count`·`vacuum_count` **0**). 방법 = 전량 실행 중 0.2초 간격 **472 표본**의 최대값 |
| `pg_catalog` `sum(autovacuum_count)` | 4(넷이 카탈로그) | **10 — 열 개 전부 `pg_catalog`**(`pg_attrdef`·`pg_attribute`·`pg_class`·`pg_constraint`·`pg_depend`·`pg_index`·`pg_namespace`·`pg_sequence`·`pg_trigger`·`pg_type` 각 1) |
| `pg_catalog` `max(n_dead_tup)` | — | **89,774**(`pg_attribute`). 기준선(빈 클러스터) **4** |
| `pg_catalog` `n_tup_del` / `n_tup_upd` | — | **405,216** / **13,761** |
| 남은 `bt_test_%` 스키마 | (지우지 않았다 — 504) | **0** |

**세 가지를 갈라 적는다.**

1. **㉠ 의 결론은 유지되고, 근거가 바뀐다.** *"같은 기전이 수백 건에 걸린다"*(ADR 0015
   §Alternatives 2 가 이 사이클에 미룬 그 걱정)는 **일어나지 않았다** — 그러나 그것을 말하는 값은
   ㉠ 의 42 도, 배선 끝의 0 도 아니고 **실행 중의 2**(문턱 50 의 4%)와 **사용자 테이블
   `autovacuum_count`=0** 이다. 42 는 다른 격리의 수이고, 0 은 수가 아니라 부재다.
2. **끝값 0 을 관측으로 읽지 않는다**(계획 0013 §후속 46 과 같은 모양 — 도구·대상의 부재를 음성
   관측으로 읽는 것). `qa` 는 그 구조를 정확히 진단해 커밋 본문에 적었고, 이 정정은 그 위에
   **실제 수**를 얹은 것이다.
3. **`qa` 의 값 중 하나는 그대로 재현되지 않았다 — 그리고 그것이 이 값의 성질을 말한다.**
   `max(n_dead_tup)`(카탈로그)이 `qa` 는 **71,724**, 나는 **89,774** 다(둘 다 `pg_attribute`).
   같은 워크로드인데 갈리는 이유는 이 값이 **autovacuum 이 도는 시점의 스냅샷**이기 때문이다 —
   같은 실행에서 결정적인 것은 워크로드가 만든 수(`n_tup_del`=**405,216**, `n_tup_upd`=**13,761**,
   autovacuum 이 돈 관계 **열 개**)이고 이 셋은 두 측정에서 **자릿수까지 같다**. 그러므로
   **71,724 도 89,774 도 계약이 될 수 없다**: 이 자리에서 못박을 수 있는 명제는 *"카탈로그의
   `n_dead_tup` 이 기본 문턱을 세 자릿수 배로 넘고, autovacuum 이 실제로 도는 자리는 전부
   카탈로그다"* 하나다.

### 8-3. 결정 4 를 **다시 열고, 같은 자리에 다시 닫는다** (§후속 30)

§3 완료 조건 4 는 *"값이 문턱(50)을 넘으면 `autovacuum` 결정을 다시 연다"* 이고, **카탈로그에서
넘었다**(89,774 ≫ 50). 그래서 **열었다.** 판정:

- **결정은 유지된다 — `autovacuum` 을 끄지 않는다**(ADR 0015 §2-3 · 이 ADR §2-4). 문턱을 넘은
  자리가 정확히 **끄면 유지가 함께 꺼지는 자리**이기 때문이다. 뒤집으려면 *사용자 테이블*의 압력이
  문턱을 넘어야 하는데, 실행 중 실측이 **2**다.
- **그러므로 「문턱을 넘었다」는 사실만으로는 이 결정이 재개되지 않는다 — 완료 조건 4 의 문안이
  그 구별을 싣지 않았다.** 그 조건을 쓴 것은 나다(§3 4). **정정**: 재개 조건은
  *"`pg_catalog` 를 뺀 관계에서 `n_dead_tup` 이 문턱을 넘거나, 사용자 테이블에서 autovacuum 이
  실제로 도는 것이 관측될 때"* 다. **다음 사이클은 이 문안으로 읽는다**(§2-4 역방향 확인의
  *"재측정이 ㉠ 을 뒤집으면"* 도 같은 뜻으로 좁힌다).
- **한정 셋.** ① 이 값은 **테스트마다 새 스키마**의 것이다 — 세션 스키마는 두 값을 반대 방향으로
  움직이고 **재지 않았다**(계획 0013 §후속 59). ② **N=1** 이다(전량 실행 하나 · 표본 472).
  ③ **PostGIS 없는 로컬 16.13** 의 값이다. CI 의 DB 는 **PostGIS 확장 넷이 로드된 16.4** 이고
  (§8-5), 카탈로그가 훨씬 크므로 카탈로그 압력은 CI 에서 **더** 크다 — 방향은 이 결정을 강화한다.

### 8-4. §후속 31 은 **닫지 않는다** — 값은 있고 물음은 답해지지 않았다

`qa` 의 진단을 **내 실행으로 재현했다**(저장소 밖 pytest 플러그인으로 그 함수의 반환만 읽었다 —
`tests/` 를 한 글자도 만지지 않았고 실행 뒤 루트 `git status --porcelain` 은 빈 출력, N=3 세 번 다
같은 값):

    재배치 전 [이른행=(0,7) 늦은행=(0,9) n_dead_tup=0 autovacuum_count=0 vacuum_count=0]
    재배치 뒤 [이른행=(1,1) 늦은행=(0,10) n_dead_tup=0 autovacuum_count=0 vacuum_count=0]

즉 **되돌아온 line pointer 가 하나도 없다**(그 테이블에서 진공이 한 번도 돈 적이 없다). 10/10 은
*"재배치가 FSM 앞 페이지 경로에서도 상대 순서를 지킨다"* 를 보인 것이 **아니고**, 그 경로가
**구조적으로 날 수 없는 조건**에서 잰 값이다. §2-3 이 *"탐침의 10/10 은 그 값이 아니다"* 라고
적었는데, **배선의 10/10 도 같은 이유로 그 값이 아니다** — 바뀐 것은 격리의 이름이지 조건이 아니다.

**판정**: §후속 31 은 **한정과 함께 열린 채 남는다.** 닫히는 조건을 값으로 못박는다 —
*"그 테이블에서 `vacuum_count` 또는 `autovacuum_count` 가 1 이상인 상태로 재배치가 일어난
실행에서 ⓑ 가 참인 비율"*. 그 조건을 만들 수 있는 것은 오늘 **세션 스키마 격리**(계획 0013
§후속 59) 하나이고, 그 둘은 이제 **한 항목의 앞뒤**다.

### 8-5. 이 ADR 의 두 자리가 **CI 실측으로 갈렸다** (반증)

1. **§Deferred 4 의 「CI 러너에서 아무것도 재지 않았다」는 이제 낡았다** — 이 사이클의 커밋이
   푸시돼 CI 가 `87d743b` 에서 초록으로 돌았다(run `34175377673`, 잡 넷 전부 success). **문장을
   소급해 고치지 않는다**(§3-13). 값은 §8-6 에 있다.
2. **`.github/workflows/buildtwin-ci.yml:82` 의 주석 *"확장은 만들지 않는다(§후속 24)"* 는
   실행값과 어긋난다.** 두 잡의 서비스 컨테이너 로그(내가 내려받은 원본)가 이렇게 적는다
   (**인용 · 발췌**): `Loading PostGIS extensions into buildtwin_test` 뒤에 `CREATE EXTENSION` **넷**.
   워크플로가 **스텝으로** 확장을 만들지 않는 것은 맞지만, **테스트가 보는 DB 에는 PostGIS 가
   들어 있다** — 이미지의 엔트리포인트가 `POSTGRES_DB` 에도 로드한다. ADR 0015 §Deferred 4 는
   같은 사실을 **옳게** 적고 있었다(*"CI 는 `postgis/postgis:16-3.4`(16.4, 확장 넷 로드)다"*) —
   **같은 저장소의 두 자리가 다르게 말하고, 새로 쓴 쪽이 틀렸다**(CLAUDE.md §6-3 11회차의 모양).
   그 파일은 `qa` 소유라 이 커밋이 고치지 않는다 — 계획 0013 §후속 **61**.
   **곁따르는 정정**: 계획 0013 §후속 24 가 *"이 계획의 클러스터에도 부재"* 라고 적은 것은
   **로컬에 대해서만** 참이다. CI 의 축은 **처음부터** PostGIS 가 로드된 DB 위에서 돌았고,
   로컬 측정은 전부(탐침 · `qa` · 이 절) 그렇지 않다 — **두 환경이 계속 달랐고 아무도 그 차이를
   재지 않았다**는 것이 §후속 24 의 오늘의 상태다.

### 8-6. CI 러너 값 (내가 원본 로그에서 읽은 것, `87d743b`, **N=1**)

| 축 | 값 |
|---|---|
| `unit` 잡 두 스텝 | **각각 606 passed** — sqlite **33.88s** ↔ postgres **72.44s**(**2.14배**) |
| `unit` 잡 스텝 벽시계(잡 목록의 타임스탬프) | sqlite **37s** ↔ postgres **76s**, 잡 전체 **218s**(`timeout-minutes: 40` 의 9%) |
| `integration` 잡 두 스텝 | **각각 226 passed** — sqlite **39.54s** ↔ postgres **42.77s**(**1.08배**) |
| 엔진 버전 | CI **16.4**(PostGIS 로드) ↔ 이 절의 로컬 **16.13**(PostGIS 없음) |
| `[db-axis]` 줄 | `unit` 잡 로그 6,145줄에 **정확히 둘**(sqlite·postgres, 둘 다 `tree=tests/unit`) · `integration` 잡 로그 1,027줄에 **정확히 둘**(둘 다 `tree=tests/integration`) |

**여기서 결정 2 의 역방향 확인이 놓친 칸이 하나 보인다.** §2-2 는 *"한 트리가 통째로 빠진 세션은
그 키가 빠져 조용히 통과한다"* 를 한계로 적었는데, CI 의 `unit` 잡은 **세 트리를 한 세션**에
돌리면서 `[db-axis]` 줄을 **하나만** 찍는다 — `tests/invariants`·`tests/regression` 은 기록기도
바닥값 키도 없다. 오늘은 그 둘이 엔진을 만들지 않아(위 §8-1 1 의 **0줄**) 결함이 아니지만,
**그 트리에 DB 를 만지는 테스트가 하나 들어오는 날 그 사실은 어디에도 찍히지 않고 어떤 바닥값도
지키지 않는다.** 그것은 이 저장소가 「조용히 죽는 것」이라 부르는 모양이고, 이 사이클이 닫은
§후속 37 과 같은 계열이다(*축은 옳게 섰는데 아무도 그것을 세지 않는다*). **이 ADR 은 그 자리를
결정하지 않는다** — 계획 0013 §후속 **60**(소유 `qa` + 문안은 architect).
