...
6. **`document_mapping` 검토요청은 여섯 단계의 생애주기를 가진다**(개정 3 — ⑥ 반려를 추가한다. 개정 2가
   ...
   **⑤ 자동 재생성에 대한 주의.** 문서가 대장에 돌아오면(예: 이번 주 대장에서 실수로 빠졌다가 다음 주 다시 실림)
   ...
   같은 구조 — 시스템 재계산은 사람의 확정을 되돌리지 않는다. 단, ADR 0001과 달리 **문서 매핑에는 확정을
   되돌리는 경로가 사람에게도 없다** — 아래 Deferred 참고, 13차 리뷰). 재생성 대상은 **확정된 적 없이** 고아가 된(고아가 되는 순간까지
...
- ~~**매핑 확정 취소(unconfirm)**~~ → **ADR 0013 으로 해소됨**(2026-09-06, 아래 반려 취소와 **함께** —
  이 항목이 요구한 대로다). 원래 기술: 문서 매핑 쓰기 경로는 generate 와 confirm 둘뿐이고 확정을 되돌리는
  엔드포인트가 없다. ADR 0001의 `CONFIRMED`는 `revoke_confirmation` 전이가 있어 cm 이 되돌릴 수 있지만,
  문서 매핑은 그 대응물이 없다 — 즉 확정도 사실상 영구다. 화면이 한동안 "확정 이후에는 사람만 되돌릴 수
  있습니다"라고 약속하고 있었으나 그 경로는 존재한 적이 없다(13차 리뷰에서 문구를 사실에 맞췄다).
  아래 반려 취소와 같은 이유로 함께 설계해야 한다 — 되돌리기는 감사 필드와 검토요청 재개, 화면 도달
  경로를 같이 만들어야 하고, 확정·반려 두 방향을 따로 만들면 또 비대칭이 생긴다.
- ~~**매핑 반려 취소(unreject)**~~ → **ADR 0013 으로 해소됨**(2026-09-06). 원래 기술: 규칙 6 ⑥은 반려를 `(activity_id, doc_id)` 쌍에 대해 **영구**로 설계했고, 10차 리뷰
  후속으로 확정 엔드포인트가 반려된 매핑을 409 `document_mapping_already_rejected`로 거절한다(그 전에는 200 을
  돌려주면서 readiness 는 그 확정을 영원히 보지 못하는 반쪽 상태를 만들었다).
  **남은 운영 위험**: CM 이 잘못 반려하면 그 쌍을 되살릴 경로가 없다. 우회는 두 가지뿐이다 — 문서 제목이 바뀌면
  `doc_id`가 바뀌어(§2-1) 새 후보가 정상 생성되고, 그 외에는 DB 를 직접 손대야 한다.
  이번에 함께 만들지 않은 이유: 되돌리기는 "매핑 행의 반려 표시 해제"만으로 끝나지 않는다 — 누가 언제 왜
  ...
  **두 항목의 해소 방식(ADR 0013).** 이 항목들이 "함께 설계해야 한다"고 적은 셋을 그대로 만든다 —
  감사 필드(`evidence.extra.cancelled_mapping_reviews` append-only), 검토요청 재개(취소가 **그 자리에서**
  새 `open` 요청을 열고 **옛 요청 행은 손대지 않는다**), 화면 도달 경로(`DocumentDetailPage` 의 취소 버튼).
  다만 필드 이름은 이 항목이 예상한 `unrejected_by`/`unrejected_at` 이 **아니다**: 확정 취소와 반려 취소가
  한 경로이므로 방향을 이름에 넣지 않고 `cancelled_by`/`cancelled_at` + `previous_decision` 으로 적는다
  (ADR 0013 규칙 3). **그리고 규칙 6 ⑥ 의 "영구"는 대체되지 않고 좁혀진다** — 재계산(대장 재업로드·수동
  재계산)에 대해서는 여전히 영구하고(ADR 0013 규칙 8 의 실측: 반려 뒤 재업로드에서 그 쌍의 open 요청
  **0건**), CM 의 명시적 취소에 대해서만 풀린다.
