...
- ~~**화면 쪽 정본 동기화가 세 줄 밀려 있다.**~~ **2026-09-06 마감 정정 — 같은 사이클의 작업 5(`b2b34c4`)가
  세 줄을 다 더했다**(실측: 이 표 46행 ↔ `KnownApiErrorCode` 46 ↔ `CODE_MESSAGES` 46, 대칭차 0).
  아래 원문은 남긴다 — **"아직 없다"가 같은 계획이 메우기로 한 부재였다**는 것이 근거다(CLAUDE.md §6-1:
  부재를 적을 때 그것을 메우는 작업이 같은 사이클에 있는지 본다). 남는 요구는 그대로다: 이 표에 행을
  더하는 사람이 그 유니온을 함께 본다(자동화는 `client.ts` 상단 TODO 의 후속).
  (아래는 원문) `KnownApiErrorCode`(`apps/web/src/api/client.ts`)에는 이번에
...
| 취소 이력 | `cancelled_mapping_reviews` (`ActivityDocumentMapping.evidence.extra.cancelled_mapping_reviews`) | 취소가 지운 값을 옮겨 담는 **append-only** 목록. 항목은 `{cancelled_by, cancelled_at, cancel_note, previous_decision, previous_reviewed_by, previous_rejected_at?, previous_rejection_note?}`. `previous_decision` 은 `"confirmed"` \| `"rejected"` — 취소 뒤에는 어느 방향이었는지가 매핑 행 어디에도 남지 않기 때문이다. **저장된 과거 기록에는 이 키가 없고 마이그레이션하지 않는다** — 읽는 쪽이 키 없음을 빈 목록으로 읽는다. **2026-09-06 마감 정정 — 이 목록에는 수명이 있다**(ADR 0013 개정 1): append-only 는 한 재계산 주기 안에서만 참이고, **그 쌍이 후보로 다시 산출되는 다음 재계산**이 미확정 매핑의 `evidence` 를 새 후보로 덮으면서 이 키를 **통째로** 지운다(실측: 확정→취소 뒤 길이 1 → 재계산 직후 키 부재). 그래서 이 목록은 **감사의 정본이 아니라 사본**이다 — 취소 한 건의 내구 기록은 ① `expert_review_logs` 행(`entity_type="activity_document_mapping"`, `entity_id="{activity_id}:{doc_id}"`; `proposal` 이 취소가 지운 반려 표시를 담는다)과 ② 취소가 열거나 갱신한 요청 행의 `conflicting_sources`(아래 `취소가 여는 요청`)이고, **닫힌 요청 행은 정본이 아니다**(재확인이 열린 확정을 취소하면 그 쌍에 닫힌 행이 하나도 없다) |
| 취소가 여는 요청 | `ReviewRequest.conflicting_sources` 의 `cancelled_review_request_id` / `cancel_note` | 취소가 그 자리에서 여는 새 `document_mapping` 검토요청이 기존 `doc_id` 키에 더해 싣는 둘 — 어느 결정을 취소한 것인가 / 왜. **2026-09-07 종결 정정(ADR 0013 개정 1·2)**: 둘 다 한정어가 붙는다. ① 취소가 **이미 열린 요청을 갱신**하는 갈래에서는 `cancel_note` 가 **덮인다** — 그 행에는 마지막 사유만 남고(실측: 같은 행 하나에 취소 둘, `'1차 취소 사유'` → `'2차 취소 사유'`), 사유 전부는 `expert_review_logs` 에만 있다. ② 같은 갈래에서 `cancelled_review_request_id` 는 **언제나 `None`** 이다(계획 0006 §후속 7 이 해소했다 — ADR 0013 §Deferred 8) — 지금 취소하는 결정을 기록한 행은 **열려 있는 그 행 자신**이라 그 물음에 답할 **닫힌 행이 없고**, 모르는 값을 지어내지 않는다(CLAUDE.md §6-4 2). 그래서 그 갈래는 닫힌 행 조회를 부르지도 않는다 — `cancel_document_mapping_review` 에서 그 조회는 `if open_review is None` **안**에 있다. *"언제나"가 기대는 것*: 그 갈래가 이 값을 **상수**로 싣는다는 것뿐이고, 그 갈래에 드는 경로의 수에 기대지 않는다. 초판(§후속 7 이전)은 **마지막으로 닫힌** 행을 실어, 닫힌 옛 행이 남아 있으면 **그 앞 결정의 행**을 가리켰다. 재계산을 기다리지 않는 이유는 실측이다: 매핑 행만 되돌리면 readiness 는 "문서 매핑 1건이 CM 검토 대기"인데 **CM 큐에는 열린 것이 없다**(ADR 0013 §Context 4) |
