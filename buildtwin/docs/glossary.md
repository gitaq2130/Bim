...
| 식별 드리프트 | `document identity drift` (`ReviewKind` += `document_identity_drift`) | 대장 원문은 그대로인데 우리 쪽 식별 규칙이 바뀌어 `doc_id`가 이동한 사건. 고아 ↔ 신규 쌍의 `title` 원문이 **같으면** rename 이 아니라 드리프트다. CM이 확정·반려한 매핑이 걸려 있으면 적재당 1건의 **확인 전용** 검토요청을 만든다 — 해소에 부수 효과가 없다(ADR 0009 §5-2·§5-3) |
