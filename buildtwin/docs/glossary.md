...
| 식별 드리프트 | `document identity drift` (`ReviewKind` += `document_identity_drift`) | 대장 원문은 그대로인데 우리 쪽 식별 규칙이 바뀌어 문서의 정체성이 흔들린 사건. 관측은 둘 — **이동**(`moved`: 이번 적재에 나타나지 않은 기존 행 ↔ 제목 원문이 글자 그대로 같은 신규 문서. **고아로 좁히지 않는다** — 시트명 변경 경로는 고아를 만들지 않는다)과 **병합**(`merged`: 한 적재에서 두 대장 행이 같은 `doc_id` 로 수렴. **새 `doc_id` 일 필요가 없다** — 운영에서 흔한 쪽은 이미 있던 id 로 수렴한다). 지문 변화는 **판정 조건이 아니라 보고 값**이다. CM이 확정·반려한 매핑이 걸려 있으면 적재당 1건의 **확인 전용** 검토요청을 만든다 — 해소에 부수 효과가 없다(ADR 0009 §5-2 개정 1·§5-3) |
| 오염된 판단 | `lost_decisions[]` / `cause` = `orphaned` / `merge_overwritten` / `merge_absorbed` | 식별 드리프트가 건드린 **사람의 판단**(확정·반려)과 그 **경위**. `orphaned`=판단이 가리키던 행이 고아가 됐다(같은 문서의 새 `doc_id` 위에서 다시 판단). `merge_overwritten`=행도 `reviewed_by` 도 살아 있는데 그 `doc_id` 가 담은 **대장 행이 바뀌었다**(승인 상태가 뒤집힐 수 있고 **다시 확정할 새 `doc_id` 가 없다** — 가장 위험한 경위). `merge_absorbed`=판단이 가리키던 행이 다른 `doc_id` 에 흡수돼 사라졌다. **셋을 뭉뚱그리면 반드시 거짓이 된다** — 검토요청 제목·경고 문구·화면 카드가 모두 이 값으로 갈리며, 모르는 값을 `orphaned` 로 떨어뜨리는 폴백은 금지다(ADR 0009 §5-2 (다)) |
