...
**절대값(리뷰어 실측, HEAD `62b8bb4` — §후속 13·14 가 요구한 형식대로 트리를 못박는다):**
...
| **CI 실행 `34098577780` job `101668172383`**(이 마감이 잡 로그에서 직접 읽었다) | sqlite 스텝 / postgres 스텝 | **205 passed** (33.92s) / **205 passed** (38.27s) |
...
흔들린 것은 A 층 **하나**이고, 그 흔들림의 크기(10회 중 2회, 그리고 원인이 **한 테스트의 한 단언**)가
"대량이 아니다"를 지지한다.
...
15. ~~바닥값이 CI 에서 실제로 그 수가 되는지 재지 않았다.~~ → **읽었다**(M-3, 아티팩트 = **181**).
    *다만* **한 번의 실행값**이고, 그 수가 실행마다 같은지는 재지 않았다.
...
19. **postgres 축의 흔들림을 이 마감이 직접 재지 않았다.** 표본은 qa 10회(2회 실패, 고치기 전) +
    qa 33회(1회 미식별) + 리뷰어 25회(0회) = **누적 58회 중 미식별 1회**이고, **이 마감의 표본은
    0 회**다 — `qa` 가 같은 트리를 병렬 편집 중이라 재실행이 그 편집과 경합했다(M-0·M-2-4).
20. **`make test` 를 clean 트리에서 다시 돌리지 못했다**(같은 이유). §M-1 의 절대값은 리뷰어 실측을
    옮긴 것이고, 이 마감이 독립으로 확인한 것은 **수집 수 821 의 첫 실행 재현** 하나다. 두 번째
    실행은 **823 passed** 였는데 그것은 `qa` 의 **미커밋** 테스트 둘을 포함한 값이라 `62b8bb4` 의
    값이 아니다(M-0 표). **`make test`·`make lint` 도 이 마감 시점에는 돌리지 않았다** — 이 마감의
    산출물이 Markdown 둘(`docs/plans/0009-*.md` · `CLAUDE.md`)뿐이라 코드 경로가 없고, qa 의 미커밋
    트리를 재는 것은 어느 커밋의 값도 아니기 때문이다.
21. **CI 초록은 1회다.** 워크플로가 이 형태로 돈 것은 run `34098577780` 한 번뿐이다.

## M-5. 후속 (본문 §후속 에 이어서 — 번호를 잇는다)

21. **postgres 축의 미식별 흔들림을, 다음 발생 때 *컨테이너 사망과 구별할 수 있게* 만든다**(qa).
    **"flake 를 없앤다"가 아니다** — 누적 58회 중 1회의 실패는 출력을 잡지 못했고, 같은 배치에서
    컨테이너가 죽어 `191 errors` 가 난 실행이 따로 있었다(qa 작업 5 보고). **재현 실패는 식별이
    아니다**: 리뷰어가 25회 연속 초록을 봤다는 것은 "없다"가 아니라 "이 표본에서 다시 안 났다"이다.
    이 항목이 요구하는 것은 **다음 발생이 어느 쪽인지 판별 가능한 상태**다 — 실패 시 서버 로그·
    `pg_stat_activity`·컨테이너 종료 코드가 함께 남으면 둘이 갈린다. (M3① = CI 실패 출력 보존은
    같은 사이클의 `qa` 가 별도로 처리한다.)
