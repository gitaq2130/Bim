...
50. **§6-1·§6-3 의 근거 표면을 잰 적이 없다**(architect). 압축 바닥값이 잰 것은 §6-2 333 + §6-4 345 =
    ...
    **[정정 — 리뷰 재심사 라운드 5] 위 표의 줄 값 둘(`264줄`·`478줄`)과 절별 자수가 규약을 틀렸다.
    소급 갱신하지 않고 정정 한 줄로 남긴다**(§3-13 첫째 갈래 — 계획 0008 `:600` 이 쓴 형태:
    *"본문 §1-b-3 은 고치지 않았다"*). **판정의 근거는 §6-5 자신이 못박은 세 값이고, architect 가
    직접 재현했다**(`git show <커밋>:buildtwin/CLAUDE.md` 를 `python3` 로 두 규약으로 나란히 셈):

    | 커밋 | §6-5 가 못박은 값(줄·문자) | `wc -l` 규약 | `split('\n')` 규약 |
    |---|---|---|---|
    | `05b7940` | 54.6% · 58.8% | **54.6%** · 58.8% ✔ | 54.8% ✗ |
    | `9989288` | 53.2% · 59.4% | **53.2%** · 59.4% ✔ | 53.3% ✗ |
    | `f40e279` | 54.0% · 61.0% | **54.0%** · 61.0% ✔ | 54.2% ✗ |

    `split('\n')` 은 개행으로 끝나는 파일에 **존재하지 않는 마지막 빈 줄**을 하나 만든다.
    **셋 다 `wc -l` 규약에서만 재현되므로 §6-5 의 규약은 `wc -l` 이고, `e3037df` 의 옳은 줄 값은
    전체 477 · §6 263 이다** — 위 표의 478/264 는 **못박은 트리에 대한 잘못된 실측**이다(문자
    31,045 · 19,951 · 비중 64.26% 는 그대로 옳다). **리뷰어 값이 맞고 내 값이 틀렸다.**
    **절별 `+1자` 도 같은 뿌리이고 여기서도 리뷰어 값이 맞다 — 산술로 닫힌다.** §6 을
    *"`## 6.` 제목 줄부터 EOF"* 로 재면 각 줄의 **끝 개행이 포함**되므로 절도 경계 개행을 포함해야
    한다: 포함한 절 합은 **669 + 7,774 + 871 + 6,262 + 958 + 3,417 = 19,951** 로 `e3037df` 의 §6 과
    **한 자리도 다르지 않고**, 뺀 합은 **19,946 으로 5 가 빈다**(절 경계 다섯). **§6-5 만 일치한
    이유가 그 산술 안에 있다** — 그 절은 EOF 로 끝나 뺄 경계 개행이 없다. 위 표의 절별 값은 **뺀
    쪽**이다.
    *새 번호를 열지 않는 이유*(리뷰어 판정을 받되 근거는 다시 적는다): 고칠 것이 **값이 아니라
    규약**이고 이 항목이 곧 그 측정의 자리라, 번호를 하나 더 열면 **규약이 두 자리에 산다** —
    §M-8-1 이 「완료 조건 표 ↔ 계약 조항」에 건 것과 같은 축 판정이다.
...
51. **세션 안 `EXPLAIN` 계측을 저장소에 둔다**(qa 소유 · architect 가 ADR 문안을 진다).
    ...
    *완료 조건 셋(그 사이클이 지운다):* ⓐ **옵트인**이어야 한다(환경 변수 없이는 아무것도 하지
    ...
    ⓒ **계약이 아니라 계측이라는 것을 그 자리에 적는다** — 계획 이름을 단언으로 굳히면
    **그 회귀가 플래너의 계획에 고정된다.** ~~ADR 0015 결정 1 이 금지한 *"저장 엔진의 선택을
    테스트의 전제로 쓰는 것"* 이 된다.~~
    **[정정 — 리뷰 재심사 라운드 5] 취소선 안은 정본에 없는 문장을 인용 표시로 실었다.**
    결정 1 의 원문은 *"저장 엔진의 **배치**"* 다 — `grep -n "저장 엔진" docs/adr/0015-*.md` 는
    **그 파일 안 히트 하나**를 내고 그것이 그 줄이다. **「선택」은 그 ADR 어디에도 없다**:
    `git grep -n "저장 엔진의 선택" 5392179 -- docs` → **히트 하나**이고 **그것이 바로
    이 줄이었다.** *잰 트리를 적는 이유*: **이 정정 자신이 그 문구를 취소선과 이 문단에 다시 싣는다**
    — 지금 트리에서 같은 명령은 히트가 늘고, 그 늘어남은 §후속 53 ②(*"이 목록을 싣는 커밋도
    센다"*)가 이름 붙인 자리다.
    **한 낱말이 아니라 금지의 대상이 갈린다**: 결정 1 이 기대값으로 두지 말라는 것은 `ctid`·rowid·물리 위치 같은 **배치**이고,
    **플래너의 계획 선택은 그 조문이 말하는 대상이 아니다.** 그래서 「선택」→「배치」로 값만 갈아
    넣으면 **문장이 다른 뜻으로 틀린다** — 이 자리는 값 교체로 닫히지 않는다.
    *같은 논지의 다른 세 자리는 옳게 적었다*
    (`git grep -n "다른 얼굴" 5392179 -- '*.md' '*.py'` → 히트 **여섯**; `__pycache__` 는 세지 않고
    **각 히트를 읽어** 가르면 **셋**이 이 논지, 나머지 셋은 §후속 17 축의 *"기댐의 다른 얼굴"* 이다.
    이 정정 뒤 같은 명령은 이 문단들 때문에 더 낸다 — 위와 같은 §후속 53 ② 의 자리다):
    §M-2-3 의 넷째 대안(`:959`)과 ADR 0016 §6 대안 8 은 **「배치」로 인용하고 「다른 얼굴」로
    한정**하며, `tests/integration/test_22_row_order_contract.py` 는 **인용 없이**
    *"결정 1 이 금지한 것의 다른 얼굴"* 로 적는다. **같은 값이 한 사이클의 네 자리에 두 모양으로
    적혔고 틀린 것은 하나다**(§6-3 11회차의 모양). 라운드 4 의 교차 확인 표(`:579`)에도 §후속 53
    재계수에도 **이 값의 칸이 없었다** — 두 목록의 축이 「20칸 사망률」이라 **인용 문구는 그 축
    밖**이었다(§6-1 ①: 생성 기준이 곧 한계다).
    **맞춘 문안 — ⓒ 가 실제로 기대는 자리는 조문이 아니라 둘이다.** 그 ADR **자신이 조문을 읽은
    자리**와 **그 읽기를 낳은 실측**이다.
    - **ADR 0015 §Deferred 의 「두 인덱스의 비용 동률을 무엇이 가르는가」 항목** —
      *"결정 1 이 요구하는 것은 「어느 인덱스인지 알 것」이 아니라 「어느 인덱스인지에 기대지 말
      것」이고, 동률이라는 실측이 그 요구를 이미 정당화한다"*. 찾는 명령:
      `grep -n "어느 인덱스인지에 기대지 말 것" docs/adr/0015-*.md` → **그 파일 안 히트 하나**.
    - **그 읽기를 낳은 실측(§2-1 의 정정 상자)** — 같은 세-술어 문장이 **같은 `cost=0.14..8.17`**
      에서 세션마다 다른 btree 를 고르므로 *"특정 인덱스 이름은 계약이 될 수 없다"*. 찾는 명령:
      `grep -n "특정 인덱스 이름은 계약이 될 수 없다" docs/adr/0015-*.md` → **그 파일 안 히트 하나**.
    **줄 번호도 서수도 쓰지 않고 그 자리에서 도는 참조로 적는다**(A-3 이 `test_22` 에서 고른 형태).
    여기서 그것이 맞는 이유는 §3-13 **둘째** 갈래가 아니다 — ADR 은 못박힌 기록물이라 줄 번호가
    허용된다(첫째 갈래). 맞는 이유는 **이 조항이 §Deferred 1(§후속 40)로 다음 사이클에 물려가는데**
    그때 인용 대상인 ADR 0015 는 살아 있고 **이 사이클에만 두 번 정정됐다**(§2-1 의 `리뷰 m6`·
    `리뷰 M1` 상자)는 것이다. **두 참조 다 파일을 적는다** — 안 적으면 이 줄 자신이 함께 히트한다
    (CLAUDE.md §2 의 `명시적 허가` 참조가 같은 자리에서 겪은 것).
    *이 정정이 바꾸지 않는 것*: ⓒ 가 요구하는 것은 **계획 이름을 적지 말라**가 아니다 —
    ADR 0016 §3 2 는 오히려 **계획마다 하나씩 `EXPLAIN` 으로 이름 붙여 적으라**고 요구한다.
    금지되는 것은 그 이름을 **기대값으로 두는 것**이고, 이 구분은 결정 1 자신의 역방향 확인
    (*"관측은 금지하지 않는다 … 금지되는 것은 그것을 기대값으로 두는 것이다"*)과 **같은 축**이다.
    **취소선의 인용을 믿는 다음 사이클은 결정 1 을 텍스트 밖으로 넓혀, 이 사이클이 세운 게이트와
    충돌하는 규칙을 물려받았을 것이다** — 그래서 §후속이 아니라 이 라운드가 고친다(§6-4 1).
...
55. **정본을 다른 자리에 실을 때 「인용인가 재서술인가」를 표시한다**(architect).
    **판정: 등록한다 — 리뷰어의 제안을 받는다. 소유는 번호를 주지 않을 근거가 못 된다.**
    §후속 54 의 역방향 확인은 *"소유가 `architect` 이고 대상이 산문이라 `qa` 항목에 넣지 않았다"*
    라 적는데, 그것은 **54 에 넣지 않을 이유**일 뿐이다 — `docs/adr/`·`docs/plans/`·`.claude/agents/`
    는 전부 `architect` 소유이므로(CLAUDE.md §2) **architect 항목으로 열 수 있었다.** 그리고
    갈림의 자리는 사이클과 함께 사라지지 않고 **재생산된다**: §Deferred 1(§후속 40)이 그 조항을
    물려받을 때 **다음 계획이 다시 그 문안을 자기 상자에 싣는다.** CLAUDE.md §6-1 의
    *"「놓칠 수 있다」고 적는 것은 커버리지가 아니다"* 가 걸리는 자리다.
    *조항*: 완료 조건·바닥값·정본 문안을 자기 상자에 싣는 사이클은 ⓐ **그 자리에서 도는 참조**를
    함께 적고(조항 번호·줄 번호·서수가 아니라 — A-3 이 `test_22` 에서 고른 형태) ⓑ **인용인지
    재서술인지 · 전체인지 발췌인지**를 표시한다. **두 문안의 일치를 보는 기계는 두지 않는다** —
    저울은 CLAUDE.md §2 의 커밋 트레일러 기각(*"아무도 검증하지 않는 기계를 하나 더 만드는 일"*)
    이고, **이 항목이 그 판정과 근거를 진다.**
    *§후속 54 와 종류가 다르다*(그래서 그 번호에 합치지 않는다): 54 의 정본은 `floor.json` 의
    **기계 판독 값**이고 복창이 숫자 넷이라 **정규식으로 닫힌다.** 여기 정본은 ADR **산문 문단**
    이고 복창은 재서술된 문단이라, 바이트 동치는 **HEAD 에서 이미 빨갛다**(아래 첫 실례).
    *이 항목이 무엇을 세는가*(§후속 53 의 세 물음에 답한다): ① 「자리」 = **조항 하나를 싣는
    문단** ② 이 목록을 싣는 커밋도 센다 ③ 인용만 하는 자리도 센다.
    *실례 셋 — 전부 이 라운드가 값으로 확인했다*:
    - **ADR 0016 §3 2 ↔ 계획 §작업 7 완료 조건 2**(§M-8-1). 계획이 *"그 문안을 그대로 싣는다"*
      라 적었는데 **문자 그대로는 아니다** — ①② 번호와 「칸마다 하나가 아니라」를 §2-3 에서 끌어와
      합쳤다. **뜻은 같고 「그대로」가 느슨할 뿐**이라 이 라운드가 문안을 고치지 않는다.
      **열린 채 남는 칸이 이것이다.**
    - **§후속 51 ⓒ** — 정본에 **없는 문장**을 인용 표시로 실었다(이 라운드의 반려). **닫혔다.**
    - **`.claude/agents/qa.md` 의 「회귀 기준」 코드블록 ↔ `tests/metrics.json`** — 여덟 키 중
      **셋만** 표시 없이 싣는 **부분 인용**이었다(A-3 이 `test_22 §4` 에서 고친 것과 같은 모양).
      **값은 오늘 것과 일치했고 이 사이클이 만든 것도 아니다.** 소유가 `architect` 라 **이 라운드가
      고쳤다** — 값 복창을 걷고 그 자리에서 도는 참조(`cat tests/metrics.json`)를 남겼다.
      **번호를 따로 열지 않은 이유**: 같은 기전(표시 없는 옮겨 적기)이고, 실례를 이 항목 안에
      두면 **세 실례가 한 자리에서 읽힌다**. **닫혔다.**
    *역방향 확인 — 이 번호가 닫지 못하는 것.* 표시는 **쓰는 시점**의 규율이라, 정본이 나중에 바뀌어
    인용이 **낡는 것**은 그대로 남는다(§6-1 12회차의 *"그 뒤에 생기는 자리"*). 이 사이클에서 그것을
    잡은 것은 **언제나 바깥에서 읽은 리뷰어**였다 — A-3 · 이 라운드의 반려 · 위 첫 실례 셋 다.
    **그 사실을 기계로 바꾸지 않는다는 것이 이 항목의 판정이고, 판정을 뒤집으려면 저울(§2)을 다시
    달아야 한다.**
...
**[정정 — 리뷰 재심사 라운드 5] 이 문단의 *"갈리지 않는다"* 가 틀렸다 — 두 규약 중 하나만
§6-5 가 못박은 세 값을 재현한다(`wc -l`).** **줄은 리뷰어의 477 · §6 263 이 맞고 내 478 · 264 가
틀렸으며, 절별 `+1자` 도 리뷰어 값이 맞다**(포함한 절 합만 §6 과 산술로 닫힌다). 재현·근거·처분은
**§후속 50 의 라운드 5 정정**에 있고, 이 문단은 소급 갱신하지 않는다(§3-13 첫째 갈래).
**「규약이 하나 다를 뿐」이 곧 「갈리지 않는다」가 아니다 — 규약을 가르는 정본이 §6-5 안에 이미
있었는데 그것을 읽지 않고 두 값을 나란히 두었다**(§6-1 ①: 무엇으로 갈랐는지를 목록 옆에 적는다).
...
## M-9-0. 라운드 5 — 이 라운드의 실측이 나온 자리 (재현 방법)

**착수 HEAD `5392179` → 마감 HEAD `3feb772`.** **DB 를 띄우지 않았다** — 이 라운드의 문서 값은
**읽기·grep·재계수**다(`git show`·`git grep`·`python3 len()`). 그러므로 20칸도 `[ORD]` 도 이 라운드가
재현하지 않았다.

**HEAD 가 편집 중에 움직였다 — 그 사실과 처분을 값으로 적는다.** 착수 시 `git status --porcelain` 은
clean 이었고, 편집 도중 `qa` 의 `tests/invariants/test_postgres_floor_recitation.py` 가 **`M` 으로
나타났다**(§후속 54 가 이름 붙인 그 파일의 docstring *「못 잡는 것」* 을 재측정으로 고치는 중이었다).
그동안은 **게이트를 돌리지 않았고 `git diff` 로 읽기만 했다** — 남의 측정이 도는 트리에서 잰 값은 내
편집의 값이 아니다(§M-6 39 · 라운드 2·4 와 같은 판단). 그 뒤 `qa` 가 **`3feb772`** 로 커밋해 트리가
다시 clean 이 됐고(내 두 파일만 `M`), **그래서 게이트를 돌렸다**(CLAUDE.md §3-1 이 묻는 것은
*"바뀔 수 있었나"* 가 아니라 *"돌렸나"* 다):

| 명령 | 실행값(트리 = `3feb772` + 내 두 파일) |
|---|---|
| `make test` | **492 · 106 · 8 · 222** + vitest **283**(28 files). 축 줄: `[db-axis] dialect=sqlite tests_on_postgres=0 engines=0 (BUILDTWIN_CI_POSTGRES_URL 없음 — postgres.measured.json 을 건드리지 않았다)` |
| 루트 한 번에(`pytest tests -q`) | **840 passed** |
| `make lint` | **exit 0** |
| 뒤의 `git status --porcelain` | 내 편집 둘만 — `M .claude/agents/qa.md` · `M docs/plans/0012-…md`(테스트가 트리를 더럽히지 않았다) |

*한정 둘*: ① 통합 갈래는 축 이름이 없어 **sqlite** 로 돈다 — **postgres 축의 222·187 은 이 실행의 값이
아니다.** ② `git add` 는 **내 두 파일을 경로로 명시**했다. 내가 만진 셋 중 **어떤 테스트도 `docs/` 를
import 하지 않고**, `.claude/agents/qa.md` 는 두 테스트가 docstring 에서 **문서로만** 가리킨다
(`grep -rn "agents/qa.md" tests/ --include='*.py'` → **히트 둘**: `tests/invariants/test_invariants.py:1` ·
`tests/e2e/test_core_flow.py:1` — 둘 다 그 파일의 **내용을 단언하지 않는다**. `--include` 없이 치면
`__pycache__` 의 `.pyc` 셋이 함께 잡힌다).

**이 문서가 못박은 grep 값은 두 HEAD 에서 같다**(`qa` 의 커밋이 `tests/` 하나만 만졌다):
`git grep -n "다른 얼굴" <HEAD> -- '*.md' '*.py'` → **여섯**, `git grep -n "저장 엔진의 선택" <HEAD> -- docs`
→ **하나** — `5392179` 와 `3feb772` 둘 다. §6 값도 `CLAUDE.md` 가 안 움직여 그대로다.

**§6 재측정(HEAD `3feb772` — `5392179` 와 한 자리도 다르지 않다. 정의는 §후속 50 과 같고
규약은 `wc -l`).** 전체 **31,387자 / 477줄**,
§6 **20,293자 / 263줄** = **64.65%(자) · 55.14%(줄)**. 절별(**경계 개행 포함** — 라운드 5 정정):
머리말 **669** · §6-1 **8,116** · §6-2 **871** · §6-3 **6,262** · §6-4 **958** · §6-5 **3,417**,
합 **20,293 = §6**(한 자리도 남지 않는다). `e3037df` 대비 자란 절은 **§6-1 하나**(7,774 → 8,116,
**+342**)이고 그것이 `1a01028`(§6-1 13회차 관측값 정정, `CLAUDE.md` 한 줄)이다. **리뷰어 값과 일치한다.**

**압축 착수는 여전히 아니다.** 못박은 표면 **678** 은 `/15,148` = **4.48%**, `/20,293` = **3.34%** —
**어느 분모로도 §6-5 바닥값의 5% 문턱 아래**다. 못박은 분모는 이제 §6 보다 **5,145자** 낡았고
(`382c746` 의 +1,237 의 네 배가 넘는다), 그 낡음은 §6-5 역방향 ④ 가 *"표면 비율을 과대평가해 과소
차단한다"* 라 적은 **불안전한 방향**이다 — **그러나 과대평가된 4.48% 조차 문턱 아래라 판정이 뒤집힐
여지가 없다.** 분모 갱신은 **§6-5 사이클이 「값과 커밋을 함께」** 한다(§M-5 *다음 사이클 순서*).
**여전히 재지 않은 것은 근거 표면**(= 규칙을 뺀 자수, §후속 **50**)이다 — 이 라운드도 **절 크기만** 쟀다.

## M-9-1. 판정 — 반려는 **값 교체로도 인용 삭제로도 닫히지 않는다**

반려는 §후속 51 ⓒ 가 ADR 0015 결정 1 을 **정본에 없는 문장으로 인용**한 것이다. 처분을 스스로 판정했다.

| 후보 처분 | 판정 | 근거(실행값) |
|---|---|---|
| 「선택」→「배치」 값 교체 | **기각** | 결정 1 이 기대값으로 두지 말라는 것은 `ctid`·rowid·물리 위치 = **배치**이고, ⓒ 가 말하는 것은 **플래너의 계획 선택**이다. 갈아 넣으면 **문장이 다른 뜻으로 틀린다**(리뷰어와 같은 판정) |
| 인용 표시를 걷고 다른 자리를 적는다 | **부분 채택** | 자리를 적는 것은 맞다. 그러나 **인용을 버릴 필요가 없다** — 그 ADR 자신이 결정 1 을 *"어느 인덱스인지에 기대지 말 것"* 으로 읽는 문단을 갖고 있다 |
| **채택 — 인용을 「그 ADR 자신의 읽기」 + 「그 읽기를 낳은 실측」 둘로 옮기고, 그 자리에서 도는 참조로 적는다** | **실행** | `grep -n "어느 인덱스인지에 기대지 말 것" docs/adr/0015-*.md` → **그 파일 안 히트 하나**(§Deferred 의 「두 인덱스의 비용 동률」 항목) · `grep -n "특정 인덱스 이름은 계약이 될 수 없다" docs/adr/0015-*.md` → **히트 하나**(§2-1 정정 상자) |

**리뷰어 주장 하나를 반증한다.** 리뷰어는 *"ADR 0015 는 그것을 금지한 적이 없다"* 라고 적었는데,
**그 ADR 의 §Deferred 는 결정 1 을 「어느 인덱스인지에 기대지 말 것」으로 읽는다** — 확장은 ⓒ 가 한
것이 아니라 **그 ADR 자신이 한 것**이다. 반려는 그래도 성립한다: 틀린 것은 **금지의 대상**이 아니라
**인용된 문장의 존재**이기 때문이다. 그리고 이 구분이 처분을 바꾼다 — 인용을 통째로 걷으면
**그 확장이 어디에 적혀 있는지가 저장소에서 사라진다.**

**전수도 한 칸 넓어진다.** 리뷰어는 *"같은 값이 같은 사이클의 두 문서에 다르게 적혔다"* 라고 적었는데,
`git grep -n "다른 얼굴" 5392179 -- '*.md' '*.py'` 는 히트 **여섯**을 내고 각 히트를 읽으면
**이 논지가 셋**이다 —
계획 `:959` · ADR 0016 §6 대안 8 · `tests/integration/test_22_row_order_contract.py:142`(**인용 없이**
*"결정 1 이 금지한 것의 다른 얼굴"*). **네 자리 중 셋이 옳고 틀린 것은 하나**이며, 옳은 셋은 전부
**「다른 얼굴」이라는 한정**을 달고 있다. 나머지 셋은 §후속 17 축의 *"기댐의 다른 얼굴"* 로 다른 값이다.

**왜 이 자리가 §후속이 아니라 반려인가**(그 판정에 선다): ⓐ **이 사이클이 만든 현재형 거짓**이고
(§6-4 1) ⓑ **다음 사이클이 물려받는 완료 조건 안**(§Deferred 1 = §후속 40)이라, 그 인용을 믿는 쪽은
결정 1 을 텍스트 밖으로 넓혀 **이 사이클이 세운 게이트(ADR 0016 §3 2 — 계획 이름을 적으라)와 충돌하는
규칙**을 물려받는다.

## M-9-2. 라운드 5 가 반증한 것 · 재지 않은 것

**반증했다**(전부 이 라운드가 직접 잰 값):

1. **내 자신의 「갈리지 않는다」**(§M-8-0). 개행 규약은 **갈린다** — §6-5 가 못박은 세 값
   (`05b7940` 54.6%·58.8% / `9989288` 53.2%·59.4% / `f40e279` 54.0%·61.0%)이 **`wc -l` 규약에서만
   재현되고** `split('\n')` 은 셋 다 어긋난다(54.8 / 53.3 / 54.2). **줄은 리뷰어 값이 맞고 내
   478/264 가 틀렸다.** 절별 `+1자` 도 같은 뿌리이고, **포함한 절 합만 §6 과 산술로 닫힌다**
   (19,951 ↔ 뺀 합 19,946). 처분은 §후속 50 의 정정(소급 갱신하지 않는다).
2. **리뷰어의 *"ADR 0015 는 그것을 금지한 적이 없다"*** — §M-9-1 의 §Deferred 인용.
3. **리뷰어의 *"두 문서"*** — 같은 논지의 자리는 **넷**이고 그중 하나가 `tests/` 다(§M-9-1).

**재지 않았다**: §M-8-3 의 아홉이 **전부 그대로 열려 있다.** 이 라운드는 DB 를 띄우지 않았고,
게이트는 돌렸지만 **어떤 변이 아래에서도 돌리지 않았으므로 ⑨(§M-6 38)는 그대로 열린다** — 돌린 것은
`docs/` 와 `.claude/` 만 바꾼 트리의 값이다. **새로 더한 미측정은 없다.** 이 라운드가 연 번호는
**§후속 55 하나**이고, 그것은 미측정이 아니라 **닫지 않기로 한 한계**다(저울: CLAUDE.md §2).
**닫힌 것은 문안 두 자리**(§후속 51 ⓒ · `.claude/agents/qa.md` 의 부분 인용)와 **정정 둘**
(§후속 50 · §M-8-0)뿐이다.
