...
> `/proc/net/tcp` 와 `/proc/net/tcp6` 에서 **상태 `0A`(LISTEN)** 인 행의 로컬 포트를 뽑고,
> 이어서 `socket.bind(("127.0.0.1", p))` 로 실제로 물어 봤다. 실행값: **55430~55450 구간에
> LISTEN 0건**, `bind` 는 **55442 에서 성공**. 관측된 사용 이력: qa 55432·55435·**55441** ·
...
46. **환경 탐침의 「도구 부재」를 「음성 관측」으로 읽지 않는다**(architect + qa). M-3-2 가 이름 붙인
    자리. `cmd 2>/dev/null | grep X` 형태는 **도구가 없을 때와 히트가 없을 때가 같은 출력**이다.
