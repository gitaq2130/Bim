...
### 같은 모양이 두 커밋 뒤에서 반대 방향으로 (재심 major-2, 2026-09-04)

`2038bbb` 는 api ↔ `test_18` 의 소유를 위처럼 명시했지만 **자기보다 앞선 커밋의 같은 위반은 보지
못했다.** `91e132a`(qa 커밋)가 `apps/web/src/test/utils.tsx` 를 고쳤는데, 이 계획 **작업 5 가 그 파일을
`frontend` 에 배정**했고(같은 표), qa 에이전트 정의의 예외는 `apps/web/src/**/*.test.ts(x)` 뿐이라
접미사가 없는 `utils.tsx` 는 매칭되지 않았다 → 형식 체크 3 FAIL.

**§6-3 ② 의 사례로 세지 않는다 — 인용을 원문대로 되돌린다(후속 4, 2026-09-04).** 이 자리는 처음에
§6-3 ②를 "규칙과 그 위반은 같은 **커밋 범위**에서 함께 태어난다"로 인용하고 "세 번째 사례"라고 적었다.
원문은 "규칙과 그 위반은 같은 **커밋**에서 함께 태어날 수 있다"이다(CLAUDE.md §6-3 "1~6 회차에서 배운
것 셋" ② — 줄 번호는 옮겨다니므로 `grep -n "규칙과 그 위반" CLAUDE.md` 로 찾는다). 그 근거는 한정어에
맞춰 실행으로 태워진 것이다 — 규칙과 그것을 어긴 표가 **둘 다 `685cbc5`**(ADR 0009 §5-2 한정어 표,
`0009-…md:858` 이 `git log -S` 로 커밋 이름까지 뒤집어 확정한 값). 그런데 여기 두 커밋은 같은 커밋이
아니고 **인접하지도 않다**: 실측 `git rev-list --parents -n 1 2038bbb` → 부모 `99dbca4`,
`git log --oneline 91e132a..2038bbb` → `2038bbb`·`99dbca4` **2건**(= 위반이 규칙보다 두 커밋 앞).
그래서 ① 소제목의 "한 커밋 뒤"와 본문의 "바로 앞 커밋"을 실측대로 고치고 ② 인용을 원문으로 되돌리고
③ **계수를 지운다** — 계수가 그 넓힌 축에만 기대고 있었으므로, 이 사이클에서 §6-3 ② 의 사례 수는 늘지
않는다(CLAUDE.md §6-3 ② 근거 줄은 `685cbc5` 한 건 그대로).

**왜 넓히는 쪽이 아니라 되돌리는 쪽인가.** 인용문의 한정어를 조용히 넓히는 것은 이 저장소가 §6-3 에서
여덟 회차 잡아 온 실패와 **같은 동작**이다 — 조건절의 한정어를 옮겨 무엇이 표 안팎에 있는지를 바꾸는
것(회차 1·2·3). 방향만 반대일 뿐이고, 넓힌 인용은 "이 사례가 규칙에 들어맞는다"는 결론을 **규칙을
고쳐서** 만든다. 게다가 여기서 넓히면 원문이 싣는 힘이 사라진다: "같은 커밋"은 **"방금 이 규칙을
썼다는 사실의 방어력은 0"** 이라는 주장인데, "커밋 범위"는 그것을 "근처 어딘가"로 희석시키면서
`685cbc5` 실측이 지지하지 않는 범위까지 끌고 간다. 하필 넓힘의 대상이 §6-3 자신의 문장이다.

*역방향 확인 — 되돌리면 무엇이 밖으로 나가는가.* "같은 커밋"으로 두면 이번 모양(규칙이 위반보다
**뒤에** 태어나 자기 앞을 되돌아보지 않은 것)은 §6-3 어느 항목에도 받을 칸이 없다 — ① 은 "규칙을 쓴
다음 개정에서 어긴다", ② 는 "같은 커밋"이라 둘 다 **앞→뒤** 방향이다. 그래도 항목을 만들지 않는다:
