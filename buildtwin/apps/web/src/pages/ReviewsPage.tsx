...
function reviewDecisionMessage(review: ReviewRequest, decision: ReviewDecision): string {
  ...
  switch (kind) {
    ...
    case "document_mapping":
      return decision === "approved"
        // 13차 리뷰 시점에는 되돌리는 API 가 정말 없어서 "확정을 취소하는 기능은 없습니다"라고 적었는데,
        // **ADR 0013 이 그 API 를 만들면서 이 문장이 거짓이 됐다**(POST …/cancel-review — 확정·반려 양쪽).
        // 서버에서 걷어낸 사실이 화면 문구에 남는 것이 이 저장소의 계열 (A) 재발이므로 그 사이클이 고친다
        // (CLAUDE.md §6-4 1). 다만 취소는 "확정을 지우는" 것이 아니라 **미확정으로 되돌려 검토 큐에 다시
        // 올리는** 것이라(ADR 0013 규칙 1·2) 문구도 그 착지점을 말해야 한다 — "취소할 수 있다"만 적으면
        // CM 은 취소가 확정 이전의 아무 일 없던 상태로 되돌린다고 읽고, 실제로는 그 쌍이 큐로 돌아와
        // 도면 승인 근거에서 내려간다. 그리고 취소해도 확정으로 되돌아가지 않는다.
        ? "승인하면 이 문서 ↔ Activity 매핑이 확정됩니다(needs_review=False) — 도면 승인 근거(readiness)로 반영됩니다. Activity 정보가 바뀌면 매핑은 확정 상태로 두고 재확인 요청만 다시 열립니다. 확정을 되돌리려면 CM이 문서 상세에서 사유를 적어 취소해야 하고, 그러면 이 쌍은 미확정(검토 대기)으로 돌아가 검토 큐에 다시 열리며 도면 승인 근거에서 내려갑니다."
        // ADR 0013 규칙 8: ADR 0007 §4-2 규칙 6 ⑥ 의 "반려는 영구하다"는 **주어가 좁아졌다** — 재계산에
        // 대해서만 영구하고 CM 의 명시적 취소로는 풀린다. 옛 문구는 "되돌릴 수 없으니"라고 단정했는데
        // 그 절이 이제 거짓이다. 그렇다고 "언제든 되돌릴 수 있다"로 뒤집으면 반대 방향의 거짓이 된다 —
        // 재계산·재업로드로는 여전히 되살아나지 않고, 되돌리는 것은 사람의 명시적 행위 하나뿐이다.
        : "반려하면 이 문서 ↔ Activity 매핑에 반려 표시가 남고 검토 큐에서 내려갑니다 — 도면 승인 근거로 쓰이지 않으며, 대장을 재업로드해도 후보로 다시 제안되지 않습니다. 되살리는 경로는 CM이 문서 상세에서 사유를 적어 반려를 취소하는 것 하나뿐이고, 그때도 확정이 아니라 미확정(검토 대기)으로 돌아갑니다.";
    ...
    <div className="source-card" data-testid="document-mapping-card">
      ...
      {mappingState === "rejected" && (
        <div className="notice strong" data-testid="rejected-notice">
          {/* ADR 0013 규칙 8: 여기 있던 "(되돌릴 수 없습니다)"가 거짓이 됐다 — 재계산에 대해서만 영구하고
              CM 의 명시적 취소로는 풀린다. 어디서 푸는지까지 말한다: 취소 버튼은 문서 상세에 있다. */}
          <strong>반려된 매핑입니다</strong> — 도면 승인 근거로 쓰이지 않으며, 대장을 재업로드해도 후보로 다시
          제안되지 않습니다. 되살리려면 문서 상세에서 CM이 사유를 적어 반려를 취소해야 하며, 그러면 미확정(검토 대기)으로
          돌아갑니다.
