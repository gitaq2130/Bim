 ...
 * 이 파일의 아래 선언은 그 정본과 **기계적으로 묶여 있지 않다**(TS 는 파이썬 상수를 import 할 수
 * 없다) — 그 어긋남을 잡는 것은 `tests/invariants/test_identity_drift_cause_contract.py` 다.
 * 검토요청 `title` 산문은 이미 경위별로 갈려 쓰이지만(`document_mapper._identity_drift_review_title`), 화면이
