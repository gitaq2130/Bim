...
const CODE_MESSAGES: Record<KnownApiErrorCode, string> = {
  ...
  // 반려된 매핑을 확정하려 할 때(전용 확정 엔드포인트·검토 큐 승인 둘 다 이 방어를 받는다).
  // **ADR 0013 이 이 code 의 뜻을 좁혔다**: 반려는 이제 CM 의 명시적 취소로 풀리므로 이 409 는
  // "영원히 불가"가 아니라 **"먼저 취소하라"** 다. 다음 행동이 실제로 있으므로 그것을 말한다 —
  // 문구가 "되돌릴 수 없다"고 말하면 그 순간 화면이 서버에 없는 제약을 지어내는 것이 된다.
  document_mapping_already_rejected:
    "이 매핑은 반려된 상태라 바로 확정할 수 없습니다. 문서 상세에서 반려를 먼저 취소해 검토 대기로 되돌린 뒤 확정하세요.",
  // ADR 0013 규칙 4·규칙 6 (가): 매핑 결정 취소에 사유가 없을 때. 지켜야 하는 것 셋 —
  //  ① "새로고침"을 말하면 안 된다 — 서버 상태는 최신이고 다시 읽어도 같은 값이다. 빠진 것은 사유뿐이다
  //     (revocation_reason_required·rejection_reason_required 와 같은 판단).
  //  ② "반려하려면"이라고 말하면 안 된다 — 취소는 반려가 아니라 반려를 **되돌리는** 일이다.
  //  ③ "확정을 되돌리려면"이라고도 말하면 안 된다 — 취소는 확정 방향에서도 반려 방향에서도 걸린다.
  // 남는 다음 행동은 하나뿐이므로 그것만 말한다: 사유를 적는다.
  cancel_reason_required: "매핑 결정을 취소하려면 사유를 입력해야 합니다. 사유를 적은 뒤 다시 시도하세요.",
  // ADR 0013 규칙 6 (나): 그 쌍에 취소할 CM 결정이 없을 때(reviewed_by is None).
  // **여기서는 "새로고침"이 실제로 답이다** — 다른 CM 이 먼저 취소했거나 애초에 검토 대기였고, 둘 다
  // 다시 읽으면 화면이 바뀐다. 위 cancel_reason_required 와 요구가 정반대인 것이, 문구 규칙이
  // 문장이 아니라 **상황**을 본다는 뜻이다(CLAUDE.md §6-4). 원인을 하나로 지어내지 않는다.
  mapping_decision_not_cancellable:
    "이 매핑에는 취소할 확정·반려가 없습니다(이미 검토 대기이거나 다른 담당자가 먼저 취소했을 수 있습니다). 화면을 새로고침해 최신 상태를 확인하세요.",
};
