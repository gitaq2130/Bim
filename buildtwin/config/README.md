...
- `ingest.yaml` — DXF 평탄화 허용오차(m) 등 도면 인식 설정 (담당: `bim-ingest`)
- `sync.yaml` — 2D↔3D 매핑 임계값·그리드 정합 허용오차·단면 기본 오프셋 (담당: `sync-2d3d`)
- `resources.yaml`, `wbs_mapping.yaml`, `activity_mapping.yaml` — CP-SAT 자원 한도, WBS·키워드 매핑 규칙 (담당: `progress-engine`)
